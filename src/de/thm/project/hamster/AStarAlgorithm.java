...
private static void goPath(ArrayList<Field> path)
{
	Hamster Ham = new Hamster();
   	for(Field f : path)
    {
    	while(f.getRow() < Ham.getRow())
    	{
    		while(Ham.getDirection() != Hamster.NORD)
    		{
    			Ham.linksUm();
    		}
    		Ham.vor();
    	}
    	while(f.getRow() > Ham.getReihe())
    	{
    		while(Ham.getBlickrichtung() != Hamster.SUED)
    		{
    			Ham.linksUm();
    		}
    		Ham.vor();
    	}
    	while(f.getColumn() < Ham.getSpalte())
    	{
    		while(Ham.getBlickrichtung() != Hamster.WEST)
    		{
    			Ham.linksUm();
    		}
    		Ham.vor();
    	...
    	while(f.getColumn() > Ham.getSpalte())
    	{
    		while(Ham.getBlickrichtung() != Hamster.OST)
    		{
    			Ham.linksUm();
    		}
    		Ham.vor();
...
private static void calcNeighbor(Field currField, Field neighbor, Field corn, int rowOffset, int columnOffset, ArrayList<Field> openList, ArrayList<Field> closedList)
{
	neighbor.setRow(currField.getRow() + rowOffset);
    neighbor.setColumn(currField.getColumn() + columnOffset);
   	boolean mauerDa = Territorium.mauerDa(neighbor.getRow(), neighbor.getColumn());
...
public static void aStarAlgorithm(Field corn)
{
    ...
    Field previous = new Field();
    
    //Corn found?
    boolean found = false;
    
    //Get the Default-Hamster
    Hamster Ham = Hamster.getStandardHamster();
    
    //Initalize start-field
    currField.setRow(Ham.getReihe());
    currField.setColumn(Ham.getSpalte());  
    
    //Save start-field as previous-field
    previous = currField;
    ...
    if(!found)
    {
    	Ham.schreib("Openlist ist leer! => Kein Weg zum Korn!");
    }
    ...
    else
    {
    	...
    	while((tempField.getRow() != Ham.getReihe()) || (tempField.getColumn() != Ham.getSpalte()))
    	{
